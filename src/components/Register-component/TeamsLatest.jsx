...
const TeamsLatest = () => {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        const response = await fetch(URL + '/api/teamlatest/getusers');
        if (!response.ok) {
                    ...
                    >
                      <div className="sg-member-photo-wrapper">
                        <div className="sg-photo-frame">
                          <img
                            src="https://wallpapers.com/images/hd/best-profile-pictures-rhntjz9x5ccnkobx.jpg"
                            alt="Member"
                            className="sg-member-photo"
                          />
                        </div>
                        <div className="sg-player-number">
                          {/* <span>{member._id.slice(-3)}</span> */}t
                        </div>
                      </div>
